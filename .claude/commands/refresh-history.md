---
description: Research and curate Historical Archive records on the Max plan ($0 API), auto-published via the trust gate — no PR.
---
...
- **Every claim must cite a real URL on an allowlisted authoritative domain.** Read the allowlist in
  `data/source-allowlist.json` first. For historical sourcing that includes the
...
1. **List existing historical slugs** to avoid duplicates:
   ...
   **Also check for unpublished work before proposing anything new:**
   ...
   before drafting: if it already covers the record you were about to add, extend that branch
